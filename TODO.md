...
- **Extend the Renode boot smoke test to cover scheduler/queue behavior.**
  ...
  `os_app_main` and that the core isn't stuck in a fault handler. Ticks
  are enabled and running, so this is no longer blocked -- extend the
  test to assert on `app/app_main.c`'s debug globals (`g_error_count == 0`,
...
- ~~Fix `GDB := aç` in the Makefile.~~ **Fixed** — was a stray typo for
  `arm-none-eabi-gdb`; corrected, `make debug`/`make renode-debug` work now.
